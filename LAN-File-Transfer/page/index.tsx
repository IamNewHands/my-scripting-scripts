...
export function ChatPage({ initialFiles }: { initialFiles?: string[] }) {
  ...
  // 关闭前二次确认：关闭会断开会话并删除已接收的文件，避免误触。
  // 确认后主动完成清理并退出，不依赖 Navigation.present 是否 resolve（真机上可能不 resolve 导致脚本残留在后台）
  async function onClose() {
    const confirmed = await Dialog.confirm({
      title: "关闭会话？",
      message: "这是关闭按钮：点击后将断开与浏览器的连接，并删除本次已接收的所有文件。要保留文件，请用旁边的「最小化」。",
      cancelLabel: "取消",
      confirmLabel: "确认关闭",
    })
    if (!confirmed) return
  ...
  return (
    <ZStack
      ...
      toolbar={{
        topBarLeading: [
          <Button title="关闭" systemImage="xmark" tint="red" action={onClose} />,
