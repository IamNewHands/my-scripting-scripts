...
function addError(msg){
  ...
  el.style.cssText = 'text-align:center;color:#ff453a;font-size:13px;padding:8px 0;';
  el.textContent = '⚠ ' + msg;
  messagesEl.appendChild(el);
}

function addMessage(m){  var wrap = document.createElement('div');
...
function uploadFile(file){
  ...
  fetch('/upload?name=' + encodeURIComponent(file.name || '未命名'), {
    ...
    .then(function(res){
      // 413 等 HTTP 错误不会进 catch，需在此显式检查
      if (!res.ok) throw new Error('服务器拒绝（HTTP ' + res.status + '）');
    })
    .catch(function(){
      addError('上传失败：' + (file.name || '文件') + '（可能过大或连接中断）');
    });
}
