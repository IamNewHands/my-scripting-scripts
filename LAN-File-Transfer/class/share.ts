...
export class Share {
  ...
  // 单次上传体积上限：放大到 1GB。HttpServer 默认 maxRequestBodySize 仅 50MB，
  // 传视频/大压缩包会被 413 拒绝（内存经 base64 中转约 ×2.6，故不宜再放大）
  ...
  async start() {
    ...
    let error = this.server.start({ port: 66666, maxRequestBodySize: 1024 * 1024 * 1024 })
    if (error) {
      this.server = new HttpServer()
      error = this.server.start({ port: 0, maxRequestBodySize: 1024 * 1024 * 1024 })
    }
    ...
    // 浏览器聊天页（异步 handler；同步版 registerHandler 已废弃，避免未来大版本移除）
    this.server.registerAsyncHandler("/", async () => HttpResponse.ok(HttpResponseBody.html(chatPageHtml())))

    // 文件上传（浏览器 → 本机）：浏览器直传原始字节，文件名走 ?name=，类型走 content-type
    // （parseMultiPartFormData 对浏览器请求返回空 part：filename=null、data=undefined，不可用）
    // 直写原始字节写盘（已实测 req.body 完整可靠，省掉 base64 中转的内存翻倍）；
    // 结果先入 inbox 队列，由页面定时 drainInbox 拉取（async handler 上下文里直接回调 UI 会导致进程崩溃）
    this.server.registerAsyncHandler("/upload", async (req) => {
      try {
        ...
        if (req.body.size === 0) {
          ...
          // 直写原始字节，避免整文件经 base64 中转（内存约 ×2.6）
