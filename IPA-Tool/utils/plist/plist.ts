...
import { parseBinary, parseXML } from "./plist-parser"
...
const parse = (input: string | Uint8Array | ArrayBuffer) => {
  ...
  if (header.startsWith("bplist")) return parseBinary(data)

  const textData = Data.fromArrayBuffer(data.slice().buffer as ArrayBuffer)
  return parseXML(textData?.toRawString() ?? "")
}
