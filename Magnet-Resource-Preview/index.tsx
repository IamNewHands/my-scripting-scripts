...
import { FavoriteItem, HistoryItem, WhatsLinkResponse, WhatsLinkScreenshot, XciliDetailInfo, XciliDetailFile, XciliSearchItem } from "./types";
import { absoluteXciliUrl, addHistory, buildMissavSearchUrl, decodeHtml, displayFileType, extractFanhao, extractPureMagnetLink, extractSupportedLink, formatBytes, getCover, isPlayableUrl, isSupportedLink, loadFavorites, loadHistory, normalizeInput, persistFavorites, saveHistory, shortLink, stripHtml } from "./utils";
import { fetchXciliDetail, fetchXciliMagnet, parseXciliDetail, parseXciliSearchResults, queryWhatsLink, searchXcili } from "./api";
import { BackButton, BLUE, CloseButton, GlassButtonContent, GlassShape, GlassVariant, glassEffectFor, glassFillFor, glassShadowFor, glassSurface, glassTintFor, GLASS_FILL, GLASS_STROKE, GLASS_TINT, INPUT_GLASS_FILL, MetaLine, SEARCH_PAGE_SIZE, SmallResultButton } from "./components/Glass";
...
// 全屏视频播放器页面
function VideoPlayerPage({ url }: { url: string }) {
    ...
    >
      ...
      >
        <Button action={dismiss}>
          <Image systemName="xmark.circle.fill" foregroundStyle="white" frame={{ width: 32, height: 32 }} />
        </Button>
      </VStack>
    </ZStack>
  );
...
// 复制磁力链接到剪贴板，打开 webtor.io 网页让用户粘贴在线播放
async function playMagnetInWebView(magnetUrl: string) {
  // 先复制磁力链接到剪贴板
  await Pasteboard.setString(magnetUrl);

  const webView = new WebViewController();
  try {
    const ok = await webView.loadURL("https://webtor.io/");
    if (!ok) {
      // 回退到系统打开
      await Safari.openURL(magnetUrl);
      return;
    }
    await webView.present({ fullscreen: true, navigationTitle: "在线播放" });
  } finally {
    webView.dispose();
  }
}
...
function PreviewCard({
  ...
  const handlePlay = () => {
    if (url.startsWith("http://") || url.startsWith("https://")) {
      void Navigation.present({ element: <VideoPlayerPage url={url} />, modalPresentationStyle: "overFullScreen" });
    } else {
      // 磁力/ED2K 链接：用 WebViewController + webtor.io embed SDK 在线播放
      void playMagnetInWebView(url);
    }
  };
    ...
    >
      ...
      {!exportMode ? (
        <HStack spacing={8} frame={{ maxWidth: "infinity" }}>
          ...
          <Button action={() => void Safari.openURL(url)} buttonStyle="plain">
            <Text font={13} fontWeight="semibold" padding={{ vertical: 7, horizontal: 10 }} {...glassSurface(14, "control", true, false)}>
              打开链接
            </Text>
          </Button>
          {isPlayableUrl(url) ? (
            <Button action={handlePlay} buttonStyle="plain">
      ...
      <VStack alignment="leading" spacing={16} padding={18} frame={{ maxWidth: "infinity" }}>
        ...
        {detail.magnet ? (
          <HStack spacing={8} frame={{ maxWidth: "infinity" }}>
            ...
            <Button action={() => void Safari.openURL(detail.magnet)} buttonStyle="plain">
              <Text font={13} fontWeight="semibold" padding={{ vertical: 7, horizontal: 10 }} {...glassSurface(14, "control", true, false)}>
                打开链接
              </Text>
            </Button>
  ...
  const handleDownloadAllScreenshots = async () => {
    ...
    try {
      for (let i = 0; i < shots.length; i += 1) {
        ...
        const res = await fetch(imageUrl, { timeout: 10 });
          ...
          <VStack alignment="leading" spacing={20} padding={18} frame={{ maxWidth: "infinity" }}>
            ...
            {loading ? (
              ...
              <VStack alignment="leading" spacing={14} frame={{ maxWidth: "infinity" }}>
                <Button action={handleBack} buttonStyle="plain">
                  <HStack spacing={8} padding={{ vertical: 8, horizontal: 0 }} frame={{ maxWidth: "infinity", alignment: "leading" }}>
                    <Image systemName="chevron.left" fontWeight="semibold" foregroundStyle={BLUE} />
                    <Text font={15} fontWeight="semibold" foregroundStyle={BLUE}>返回</Text>
                  </HStack>
                </Button>
                <PreviewCard
                  key={currentUrl}
